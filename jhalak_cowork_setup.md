...
Skip any row where:
- AM = "Y" (halted)
- AK = "Y" (already replied)
- A is blank (no prospect ID)

These should already be filtered out by the sheet formulas, but double-check.
...
Personalize by prospect tier (from the prospect's designation/org type):
- Wealth Manager: lead with "Your clients earn 3-4% residential yield" / pre-empt capital protection
...
| Tier personalization | Email angle matches prospect type (WM = yield pain, NRI = tax-free + no presence, FO = operating income gap) | |
...
- **AO** (Next Action) — what to do next
- **AP** (Next Action Date) — when the next action is due
- **AQ** (Action Due?) — TODAY, OVERDUE, or FUTURE
- **AR** (Status) — overall prospect status
...
The AQ (Action Due?) formula compares AP against TODAY() and returns "OVERDUE", "TODAY", or "FUTURE".
...
The daily outreach task filters on AQ = "TODAY" or AQ = "OVERDUE" to find actionable rows.
